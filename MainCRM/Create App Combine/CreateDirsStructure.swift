...
struct CreateDirsStructure: View {
        ...
        if manager.fileExists(atPath: FConfig.mainDir) {
            do {
                try manager.createDirectory(
                    atPath: combineVM.assetsPath,
                    withIntermediateDirectories: true
                )
                try manager.createDirectory(
                    atPath: combineVM.imagessPath,
                    withIntermediateDirectories: true
                )
                try manager.createDirectory(
                    atPath: combineVM.configsPath,
                    withIntermediateDirectories: true
                )
                try manager.createDirectory(
                    atPath: combineVM.screensPath,
                    withIntermediateDirectories: true
                )
                isSuccess = true
            } catch {
